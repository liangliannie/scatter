...
class Sino_Dataset(Dataset):
    ...
    def loadFile(self, data_file="training_data.npy"):
        ...
        try:
            with open(data_file, 'rb') as file:
                ...
                self.data = np.stack([data['target'], data['input']]).astype('float32')
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, slice):
            ...
            # mask_sino = self.mask[idx:idx+self.channelnumber]

        self.DataQueryCount += 1

        return orig_sino, corrupt_sino#, mask_sino
